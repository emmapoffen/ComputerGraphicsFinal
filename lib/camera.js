...
var Camera = function(aspect) {
    ...
    this.far = 100.0;             // The distance to the far plane
    ...
    this.lookAt( vec3.fromValues(-10.4,.7,-1), vec3.fromValues(0,0,0), vec3.fromValues(0,1,0) );
...
Camera.prototype.dolly = function(delta) {
    ...
    let x = vec3.fromValues( this.rotation[0], this.rotation[4], this.rotation[8] );
    let n = vec3.create();
    vec3.cross( n, up, x );
